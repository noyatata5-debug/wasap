...
-- 7. Tabel Assets (Aset & Tabungan: Emas, Crypto, Saham/BBCA, Tabungan Bank, Reksadana, dll)
CREATE TABLE IF NOT EXISTS public.assets (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  phone_number TEXT,
  name TEXT NOT NULL,                 -- e.g. "Emas Antam 10g", "Bitcoin (BTC)", "BBCA (Bank Central Asia)", "Tabungan BCA"
  category TEXT NOT NULL DEFAULT 'emas', -- 'emas' | 'crypto' | 'saham' | 'tabungan' | 'reksadana' | 'lainnya'
  amount BIGINT NOT NULL DEFAULT 0,  -- Nilai total saat ini dalam IDR
  buy_amount BIGINT DEFAULT 0,       -- Modal beli / harga perolehan dalam IDR (opsional untuk PnL)
  quantity TEXT,                     -- e.g. "10 gram", "5 lot", "0.05 BTC", "1 Rekening"
  institution TEXT,                  -- e.g. "BCA", "Ajaib", "Indodax", "Bibit", "Brankas Pribadi"
  notes TEXT,                        -- Catatan tambahan
  asset_date DATE DEFAULT CURRENT_DATE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);

-- 8. Migrasi Kolom Tambahan (Idempotent)
...
ALTER TABLE public.assets ADD COLUMN IF NOT EXISTS phone_number TEXT;
ALTER TABLE public.assets ADD COLUMN IF NOT EXISTS buy_amount BIGINT DEFAULT 0;
ALTER TABLE public.assets ADD COLUMN IF NOT EXISTS quantity TEXT;
...
CREATE INDEX IF NOT EXISTS idx_assets_phone ON public.assets(phone_number);
